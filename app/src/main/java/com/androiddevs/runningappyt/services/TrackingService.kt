...
import com.androiddevs.runningappyt.other.TrackinggUtility
...
class TrackingService : LifecycleService() {
    ...
    companion object {
        val timeRunInMiliis = MutableLiveData<Long>()
        val isTracking = MutableLiveData<Boolean>()
        val pathPoints = MutableLiveData<Polylines>()
    }

    private fun postInitalValue() {
        isTracking.postValue(false)
        pathPoints.postValue(mutableListOf())
        timeRunInSeconds.postValue(0L)
        timeRunInMiliis.postValue(0L)
    }
    ...
    private fun startTimer(){
        ...
        CoroutineScope(Dispatchers.Main).launch {
            while (isTracking.value!!){
                ...
                timeRunInMiliis.postValue(timeRun + lapTime)
                if(timeRunInMiliis.value!! >= lastSecondTimestamp + 1000L){
    ...
    private fun updateLocationTracking(isTracking : Boolean){
        if (isTracking){
            if (TrackinggUtility.hasLocationPermissions(this)){
        ...
        timeRunInSeconds.observe(this, Observer {
            if (!serviceKilled){
                val notification = currentNotificationBuilder
                    .setContentText(TrackinggUtility.getFormattedStopWatchTime(it * 1000L))
